...
def evaluate_model(
    ...
    """Evaluate a fitted pipeline and return a metrics dict.
    ...
    """
    logger.info(
        "[evaluate] Starting evaluation on %d samples",
        len(y_eval),
    )
    ...
    if problem_type == "classification":
        ...
        metrics["precision_0"] = float(
            precision_score(
                y_eval, y_pred, pos_label=0, zero_division=0
            )
        )
        metrics["recall_0"] = float(
            recall_score(
                y_eval, y_pred, pos_label=0, zero_division=0
            )
        )
        metrics["precision_1"] = float(
            precision_score(
                y_eval, y_pred, pos_label=1, zero_division=0
            )
        )
        metrics["recall_1"] = float(
            recall_score(
                y_eval, y_pred, pos_label=1, zero_division=0
            )
        )
        ...
        if hasattr(model, "predict_proba"):
            ...
            metrics["roc_auc"] = float(
                roc_auc_score(y_eval, proba)
            )
            metrics["pr_auc"] = float(
                average_precision_score(y_eval, proba)
            )
            metrics["brier"] = float(
                np.mean((proba - y_eval) ** 2)
            )
        ...
        logger.error(
            "[evaluate] Unsupported problem_type: '%s'.",
            problem_type,
        )
        raise ValueError(
    ...
    logger.info(
        "[evaluate] Done | accuracy=%.4f | f1=%.4f "
...
def make_plots(model, X_eval, y_eval):
    """Return a Figure with PR curve and confusion matrix."""
    proba = model.predict_proba(X_eval)[:, 1]
    ...
    sns.heatmap(
        cm, annot=True, fmt="d",
        cmap="Blues", cbar=False, ax=ax_cm,
    )
